...
function PlayIcon({ className = "w-4 h-4" }: { className?: string }) {
  return (
    <svg viewBox="0 0 24 24" className={`${className} fill-current ml-0.5`}>
  ...
  return (
    <div className={`group flex items-center gap-3 p-3 rounded-2xl border transition-all duration-200 ${
      ...
      <div className="flex items-center gap-1 flex-shrink-0">
        ...
        <button
          onClick={onDelete}
          className="w-8 h-8 rounded-full text-gray-700 hover:text-red-400 hover:bg-red-950/40 flex items-center justify-center transition-colors text-lg leading-none"
        >
          ×
        </button>
      </div>
  ...
  return (
    <div className="fixed bottom-16 left-0 right-0 z-20 px-3 pb-2 max-w-lg mx-auto">
      <div className="bg-gray-900/95 backdrop-blur-xl border border-gray-700/40 rounded-2xl p-3 flex items-center gap-3 shadow-2xl shadow-black/50">
        ...
        <button
          onClick={onStop}
          className="w-10 h-10 rounded-full bg-gray-800 hover:bg-red-950/60 border border-gray-700/50 hover:border-red-800/50 flex items-center justify-center text-gray-400 hover:text-red-400 transition-all"
        >
          <StopIcon />
        </button>
      </div>
...
export default function MusicaPage() {
  ...
  async function buscar(e: React.FormEvent) {
    ...
    try {
      ...
      setErrBus("No se pudo buscar. Comprueba tu conexión.");
    } finally {
      setBuscando(false);
    }
  }
    ...
    try {
      const res = await fetch("/api/musica", {
        ...
        body: JSON.stringify({
          ...
          artista:      r.artista,
          url_youtube:  r.url,
          duracion_seg: r.duracion_seg || null,
          accion,
        }),
      });

      if (res.status === 409) {
        // Ya existe: solo enviar la acción si es play
        if (accion === "play") {
  ...
  return (
    <div className="min-h-screen bg-gray-950 max-w-lg mx-auto">
      ...
      {/* Contenido */}
      <div className={`p-4 ${jugando ? "pb-36" : "pb-24"}`}>
        ...
        {tab === "buscar" && (
          <div className="space-y-4">
            ...
            {errBusqueda && (
              <p className="text-red-400 text-xs px-1">{errBusqueda}</p>
            )}
            ...
            {buscando ? (
              ...
              <div className="space-y-2">
                <p className="text-xs text-gray-600 px-1">{resultados.length} resultados · toca ▶ para reproducir en el laptop</p>
                {resultados.map((r) => (
                  <ResultadoCard
                    key={r.id}
                    r={r}
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm mt-1">
                    Escribe el nombre de un artista o canción arriba
                  </p>
                </div>
          ...
          listas.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm mt-1">Busca y descarga canciones desde la pestaña Buscar</p>
              </div>
              <button
          ...
          pendientes.length === 0 ? (
            ...
            <div className="space-y-2">
              <p className="text-xs text-gray-600 px-1 mb-3">
                El laptop descargará estas canciones automáticamente
              </p>
              {pendientes.map((c) => (
                <CancionCard
                  key={c.id}
      ...
      {jugando && <NowPlayingBar cancion={jugando} onStop={stop} />}
